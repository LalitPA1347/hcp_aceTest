...
import { setPatientAnalyticsChartData } from "../../../../redux/descriptiveInsights/patientAnalyticsSlice";
import { DIGraphGenerateApi } from "../../../../services";
import { filterReportData } from "../../Helper";
import "./PatientAnalyticsIndication.css";
...
const PatientAnalyticsIndication = () => {
  ...
  const savedDropdown = useSelector(
    (state) => state.patientAnalyticsChart.data || {}
  );
  ...
  useEffect(() => {
    if (flag in savedDropdown) {
      ...
      if (savedDropdown?.[flag].dropDownOptions.length === 0) {
        ...
        const { reportedSelectedValue, reportedDropDownValue, reportedData } = filterReportData(
  ...
  const handleChange = (event) => {
    ...
    if (
      ...
      sortedSelectedValue["Group By Column"] &&
      sortedSelectedValue["Group By Column"].length > 2
    ) {
      toast.warning("User can select only two values");
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    if (response?.data?.Graph_Data) {
      ...
      dispatch(
        setPatientAnalyticsChartData({ ...savedDropdown, ...savedData })
      );
      setShowLoader(false);
      return;
    } else {
  ...
  const SubmitBtnDisable = () => {
    if (flag === "dot") {
      ...
      if (lastKey === "End Year" || lastKey === "Group By Column") {
        return false;
      }
      return true;
    }
  ...
  return (
    ...
    >
      <Stack
        className="Patient-analytics-indication-model"
        justifyContent="space-between"
        direction="row"
      >
        <Box>
...
export default PatientAnalyticsIndication;
