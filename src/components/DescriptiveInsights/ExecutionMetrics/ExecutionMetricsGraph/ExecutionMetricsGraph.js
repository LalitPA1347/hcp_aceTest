...
import "./ExecutionMetricsGraph.css";
import {
  Chart as ChartJS,
  CategoryScale,
  LinearScale,
...
const ExecutionMetricsGraph = () => {
  const { flag } = useParams();
  const svgRef = useRef(null);
  const chartRef = useRef(null);
  const [chartData, setChartData] = useState({});
  const data = useSelector((store) => store.executionMetricsChart.data || {});
...
export default ExecutionMetricsGraph;
