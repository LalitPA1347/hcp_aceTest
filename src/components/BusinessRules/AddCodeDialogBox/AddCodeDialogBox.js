...
const AddCodeDialogBox = (props) => {
  const { open, close, text, handleAddCode, codeName } = props;
  ...
  const handleAllProductCodeApi = async () => {
    ...
    if (response && response?.data && response.status === 200) {
      ...
      const list = data?.[codeName]?.map((obj) => {
  ...
  useEffect(() => {
    if (dialogData?.[codeName]?.length > 0) {
      const keys = deleteData?.[codeName].map((obj) => Object.keys(obj)[0]);
      const filteredAddedData = dialogData?.[codeName]?.filter(
      ...
      const list = dialogData?.[codeName]?.map((obj) => {
        const [key, value] = Object.entries(obj)[0];
        return { title: `${key}: ${value}` };
      });
      setDropDownProductCode(list);
  ...
  const handleAddProduct = () => {
    ...
    const code = showOptionArray.map((str) => str.split(":")[0]);
    const savedCode = showSelectedProduct.map((str) => str.split(":")[0]);
    const filteredArray = dialogData?.[codeName]?.filter(
      (item) => ![...code, ...savedCode].includes(Object.keys(item)[0])
    );
    const list = filteredArray?.map((obj) => {
      const [key, value] = Object.entries(obj)[0];
      return { title: `${key}: ${value}` };
    });
  ...
  const handleRemoveProduct = (code) => {
    const codeList = showSelectedProduct.filter((item) => item !== code);
    const filtercode = codeList.map((str) => str.split(":")[0]);
    const filteredArray = dialogData?.[codeName]?.filter(
      (item) => !filtercode.includes(Object.keys(item)[0])
    );
    const list = filteredArray?.map((obj) => {
      const [key, value] = Object.entries(obj)[0];
      return { title: `${key}: ${value}` };
    ...
    setDropDownProductCode(list);
    setShowSelectedProduct(codeList);
  };
      ...
      <Stack className="add-dialog-model">
        ...
        <Stack direction="row" justifyContent="flex-end">
          ...
          <Button
            ...
            onClick={() => handleAddCode(showSelectedProduct)}
          >
            Add Codes
          </Button>
        </Stack>
...
export default AddCodeDialogBox;
