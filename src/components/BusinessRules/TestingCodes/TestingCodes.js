...
import { fetchDataAtom, allTestingCodeRules } from "../../../atom";
import { useAtom } from "jotai";
import CloseIcon from "@mui/icons-material/Close";
import EditIcon from "@mui/icons-material/Edit";
import Validation from "../../../validation/Validation";
import { toast } from "react-toastify";
import "./TestingCodes.css";
...
const TestingCodes = () => {
  const [testingCodeData, setTestingCodeData] = useAtom(fetchDataAtom);
  ...
  const [testingCodeRules, setTestingCodeRules] = useAtom(allTestingCodeRules);
  const [editTestingCodeIndex, setEditTestingCodeIndex] = useState();
  const [error, setError] = useState({});
  const [inputValue, setInputValue] = useState("");

  const rows = testingCodeData.productName.flatMap((item) =>
    Object.values(item).flatMap((entries) =>
      entries.flatMap((entry) => entry.Procedure_Code || [])
    )
  );
  ...
  const generateQuery = () => {
    if (testingCodes.length === 0 || testingType.trim() === "") {
      ...
      const codeSentence = `Testing Codes in (${testingCodes
        .map((code) => `${code}`)
        .join(", ")}) THEN Testing Type is ${testingType}`;
      ...
      if (
        ...
        toast.error("Testing Code already exist!", {
          autoClose: 1000,
        });
        return;
      }
        ...
        <Stack className="testing-code-table-model">
          ...
          >
            <Stack className="testing-code-selection-box">
              <Typography sx={style.insideText}>Testing Codes</Typography>
              <Stack
                direction="row"
                alignItems="center"
                justifyContent="space-between"
              ...
              >
                ...
                {testingCodes.length === 0 && <Typography sx={{...style.addedText, color:"#979797"}}>No code Selected</Typography>}
                {testingCodes &&
            ...
            <Stack className="testing-code-selection-box">
              {testingCodeRules.length === 0 ? (
                <h2>Added Testing Codes and types will be shown here</h2>
              ) : (
