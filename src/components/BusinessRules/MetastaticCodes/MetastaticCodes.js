...
const MetastaticCodes = () => {
  const [metastaticCodeData, setMetastaticCodeData] = useAtom(fetchDataAtom);
  ...
  const DeleteText = {
    title: "Metastatic Code?",
    msg: "Are you sure you want to delete this Metastatic Code? In case you want to include it in the future, use Add functionality.",
  };
  const AddDialogBox = {
    title: "Select Codes",
  ...
  const rows = metastaticCodeData.metastaticCode.map((item, index) => {
  ...
  const handleOnRowSelectionModelChange = (ids) => {
    ...
    const deleteRow = selectedRowsData.map((item) => {
      return metastaticCodeData.metastaticCode.find((element) => element[item]);
    });
  ...
  const handleAddProductFoundInDeleteCode = (code) => {
    const filteredMetastaticCode = deleteData.metastaticCode.filter((item) =>
      code.includes(Object.keys(item)[0])
    );
    const filteredDeleteData = deleteData.metastaticCode.filter(
      (item) => !code.includes(Object.keys(item)[0])
    );
    setDeleteData((prevState) => ({
      ...prevState,
      metastaticCode: filteredDeleteData,
    }));
    return filteredMetastaticCode;
  };

  const handleAddProductnotFoundInDeleteCode = (code) => {
    const addedProduct = code.map((item) => {
      return addDialogProductData?.metastaticCode.find(
        (element) => element[item]
      );
    });
    setAddedData((prevState) => ({
      ...prevState,
      metastaticCode: [...prevState.metastaticCode, ...addedProduct],
    }));
    return addedProduct;
  };

  const handleAddCode = (codeArray) => {
    const metastaticCodeAddedData = [];

    const code = codeArray.map((str) => str.split(":")[0]);
    const filteredDialogData = addDialogProductData.metastaticCode.filter(
      (item) => !code.includes(Object.keys(item)[0])
    );

    const { found, notFound } = code.reduce(
      (acc, item) => {
        deleteData.metastaticCode.some((obj) => obj[item])
          ? acc.found.push(item)
    ...
    if (found.length > 0) {
      ...
      metastaticCodeAddedData.push(...value);
    }
    ...
    setAddDialogProductData((prevState) => ({
      ...prevState,
      metastaticCode: filteredDialogData,
    }));

    setMetastaticCodeData((prevState) => ({
      ...prevState,
      metastaticCode: [
        ...metastaticCodeData.metastaticCode,
        ...metastaticCodeAddedData,
      ],
    }));
    handleAddDialogBox();
    setConfirmationDialogBox({
  ...
  const handleDeleteProductFoundInAddedCode = (code) => {
    const filteredAddedData = addedData.metastaticCode.filter(
      (item) => !code.includes(Object.keys(item)[0])
    );
    setAddedData((prevState) => ({
      ...prevState,
      metastaticCode: filteredAddedData,
    }));
  };
  ...
  const handleDeleteProductnotFoundInAddedCode = (code) => {
    ...
    setDeleteData((prevState) => ({
      ...prevState,
      metastaticCode: [...prevState.metastaticCode, ...deletedProduct],
    }));
  };

  const handleDeleteCode = () => {
    const deleteCode = deleteRowValue.map((obj) => Object.keys(obj)[0]);
    const filteredDialogData = metastaticCodeData.metastaticCode.filter(
    ...
    setAddDialogProductData((prevState) => ({
      ...prevState,
      metastaticCode: [...prevState.metastaticCode, ...deleteRowValue],
    }));
...
export default MetastaticCodes;
